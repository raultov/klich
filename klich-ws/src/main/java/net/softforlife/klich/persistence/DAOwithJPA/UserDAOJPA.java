...
public class UserDAOJPA extends GenericDAOWithJPA<Tuser, Integer> implements UserDAO {
	...
	@Override
	public List<Tuser> getUsersRol(Trole rol) {
        Map<String, Object> params = new HashMap<String, Object>(0);
        params.put("roleId", rol.getRoleId());
        StringBuilder strQuery = new StringBuilder();
        strQuery.append("select u from Tuser u, TuserTrole t where t.roleId.roleId = :roleId and u.userId = t.userId.userId");
        strQuery.append(" ORDER BY u.userId");
        
        @SuppressWarnings("unchecked")
		List<Tuser> users = getJpaTemplate().findByNamedParams(strQuery.toString(), params);
